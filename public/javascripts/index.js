...
function extractData(data){
	...
	$.each(sample.response.responses[0].textAnnotations, function(index, object){
	...
	$.each(keep, function(index, object){
		console.log(object);
	});
	...
	// return result
	var result = { "message" : "Sorry - something went wrong.", "fields" : [] };
	
	// check number of objects (expecting 4)
	if (sorted.length > 4) {
		// TODO: look for items that should be merged into one
		result = { "message" : "We had some trouble - check the results below.", "fields" : text.slice(0, 4) };
	} else if (sorted.length < 4) {
		// fill in blanks but alert user
		result = { "message" : "We had some trouble - check the results below.", "fields" : text };
	} else {
		// everything looks good
		result = { "message" : "", "fields" : text };
	}
